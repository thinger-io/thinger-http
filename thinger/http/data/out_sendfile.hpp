...
struct sendfile_op
{
    std::shared_ptr<thinger::asio::tcp_socket> sock_;
    int fd_;
...
public:
    ...
    void to_socket(std::shared_ptr<thinger::asio::socket> socket, ec_size_handler handler) override{
        auto tcp_socket = std::dynamic_pointer_cast<thinger::asio::tcp_socket>(socket);
        if(tcp_socket){
