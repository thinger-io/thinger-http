...
namespace thinger::http{
    ...
    boost::tribool response_factory::consume(char input, bool head_request) {
            ...
            case chunked_content_expecting_n:
                if(input=='\n'){
                    if(lastChunk_){
                        ...
                        if(on_streaming_ && !resp->get_content().empty()){
                            streaming_downloaded_ += resp->get_content().size();
                            // For chunked, total is 0 (unknown)
                            if(!on_streaming_(resp->get_content(), streaming_downloaded_, 0)){
                                streaming_aborted_ = true;
                                return false;
                            }
                            resp->get_content().clear();
                        }
                        // Legacy chunked callback
                        else if(on_chunked_){
    ...
    bool response_factory::is_char(char c) {
        return c >= 0 && c <= 127;
    }

    bool response_factory::is_ctl(char c) {
        return (c >= 0 && c <= 31) || (c == 127);
    }

    bool response_factory::is_tspecial(char c) {
        switch (c) {
            case '(':
            case ')':
            case '<':
    ...
    bool response_factory::is_digit(char c) {
        return c >= '0' && c <= '9';
    }

    bool response_factory::is_hexadecimal(char c){
        return (c >= '0' && c <= '9') || (c >= 'a' && c<='f') || (c>='A' && c <= 'F');
    }

    uint8_t response_factory::get_hex_value(char c)
    {
        if(c <= '9'){
            return c - '0';
        }else if(c<='F'){
    ...
    std::shared_ptr<http_response> response_factory::consume_response() {
    ...
    void response_factory::reset() {
        ...
        headers_size_ = 0;
        on_chunked_ = nullptr;
        on_streaming_ = nullptr;
        streaming_downloaded_ = 0;
        streaming_aborted_ = false;
    }

    void response_factory::on_http_status_code(unsigned short status_code){
        resp->set_status(status_code);
    }

    void response_factory::on_http_reason_phrase(const std::string& reason){
        resp->set_reason_phrase(reason);
    }

    void response_factory::on_http_major_version(uint8_t major)
    {
        resp->set_http_version_major(major);
    }

    void response_factory::on_http_minor_version(uint16_t minor){
    ...
    void response_factory::on_http_header(const std::string& name, const std::string& value){
    ...
    void response_factory::on_content_data(char content){
        resp->get_content().push_back(content);
    }

    size_t response_factory::get_content_length(){
    ...
    int response_factory::get_status_code() const {
        return resp ? resp->get_status_code() : 0;
    }

    void response_factory::setOnChunked(const std::function<void(int, const std::string&)>& onChunked){
        on_chunked_ = onChunked;
    }

    void response_factory::setOnStreaming(std::function<bool(const std::string_view&, size_t, size_t)> callback){
        on_streaming_ = std::move(callback);
        streaming_downloaded_ = 0;
        streaming_aborted_ = false;
    }
